"""Governed V3 execution route for one 100-row V9 product-gate batch."""
...
from .classifier_product_gate_batch_plan import (
    BATCH_PLAN_CONTRACT,
    SELECTION_KIND,
...
def _load_batch_plan(path: Path, digest: str, root: Path, authorization: dict) -> dict:
    ...
    if (
        coverage_source["coverage_cohort_id"] != authorization["coverage_cohort_id"]
    ...
    matches = [
        batch for batch in plan["batches"] if batch["batch_id"] == authorization["batch_id"]
    ]
    if len(matches) != 1:
...
PRODUCT_GATE_BATCH_V3_ROUTE = PilotRunRoute(
    run_kind="classifier_product_gate_batch_v3",
    records_filename="universe_classifier_product_gate_batch_v3_records.jsonl",
    manifest_filename="universe_classifier_product_gate_batch_v3_manifest.json",
    raw_responses_filename="universe_classifier_product_gate_batch_v3_raw_responses.jsonl",
    manifest_contract="universe_classifier_product_gate_batch_manifest@0.3.0",
    manifest_schema="schemas/universe_classifier_product_gate_batch_manifest.v3.schema.json",
    authorization_contract="universe_classifier_product_gate_batch_authorization@0.3.0",
    authorization_schema="schemas/universe_classifier_product_gate_batch_authorization.v3.schema.json",
    run_root_name="universe-classifier-product-gate-batch-v3-runs",
    selection_contract=BATCH_PLAN_CONTRACT,
    selection_kind=SELECTION_KIND,
    selection_source="batch_plan",
    load_selection=_load_batch_plan,
...
def run_product_gate_batch_v3(
    *,
    ...
    return _run_lineage_classifier_pilot(
        route=PRODUCT_GATE_BATCH_V3_ROUTE,
        repo_root=repo_root,
        ...
        selection_path=batch_plan_path,
...
def require_product_gate_batch_run_v3(run_dir: str | Path) -> Path:
    return _require_pilot_run(run_dir, route=PRODUCT_GATE_BATCH_V3_ROUTE)
