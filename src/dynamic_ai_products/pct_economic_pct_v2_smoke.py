"""Five-firm smoke for economic PCT V2 with task-family-to-task output."""
...
from .pct_economic_pct_v2 import (
    OUTPUT_CONTRACT,
    OUTPUT_SCHEMA,
...
PROMPT_PATH = "prompts/extraction/pct_item1_economic_pct_v2.md"
RUN_KIND = "pct_item1_economic_pct_smoke_v2"
RAW_FILENAME = "pct_item1_economic_pct_v2_raw_responses.jsonl"
RECORDS_FILENAME = "pct_item1_economic_pct_v2_records.jsonl"
MANIFEST_FILENAME = "pct_item1_economic_pct_v2_manifest.json"
HTML_FILENAME = "pct_item1_economic_pct_v2_human_review.html"
...
def _html(records: list[dict[str, Any]], run_id: str) -> str:
    ...
    chunks = [
        ...
        "<title>Economic PCT V2 review</title>",
        ...
        f"<h1>Economic PCT V2 smoke: {esc(run_id)}</h1>",
        "<p>Development-only Item 1 smoke. Task families group distinct tasks; it settles no score, tier, transformation depth, or universe membership.</p>",
    ]
    for record in records:
        ...
        for product in record["snapshot"]["economic_products"]:
            chunks.append(f"<h3><code>{esc(product['id'])}</code> {esc(product['name'])}</h3>")
            tasks = {task["id"]: task for task in product["tasks"]}
            for family in product["task_families"]:
                chunks.append(f"<h4><code>{esc(family['id'])}</code> {esc(family['name'])}</h4><ul>")
                for task_id in family["task_ids"]:
                    task = tasks[task_id]
                    chunks.append(
                        f"<li><code>{esc(task['id'])}</code> {esc(task['text'])} — "
                        f"{esc(', '.join(task['capability_ids']))}; "
                        f"{esc(', '.join(task['passage_refs']))}</li>"
                    )
                chunks.append("</ul>")
...
def run_economic_pct_v2_smoke(
    *, plan: list[dict[str, Any]], packets_by_key: dict[tuple[str, str], dict[str, Any]],
    ...
    """Run the direct successor on saved discovery maps without reusing V1 tasks."""
    if dry_run:
        return {
            "run_id": run_id, "dry_run": True, "status": "dry_run",
            "selected_rows": len(plan), "model_called_rows": len(plan), "run_dir": None,
        }
    if generate is None:
        raise ValueError("A live economic PCT V2 smoke requires generate.")
    ...
    for row in plan:
        ...
        try:
            snapshot = validate_economic_pct_output_v2(
            ...
            records.append({
                **common, "record_kind": "extracted", "reason": None,
                "detail": None, "snapshot": snapshot,
            })
    ...
    manifest = {
        ...
        "prompt_template_path": PROMPT_PATH, "prompt_template_sha256": prompt_sha256,
        "output_contract": OUTPUT_CONTRACT, "output_schema_sha256": schema_sha256,
        "discovery_records_sha256": discovery_records_sha256, "model": model,
    ...
    try:
        for filename, payload in (
            (RAW_FILENAME, raw_bytes), (RECORDS_FILENAME, records_bytes),
            (HTML_FILENAME, html_bytes), (MANIFEST_FILENAME, manifest_bytes),
        ):
            write_bytes_once(run_dir / filename, payload, what=f"economic PCT V2 {filename}")
...
def main() -> None:
    ...
    result = run_economic_pct_v2_smoke(
