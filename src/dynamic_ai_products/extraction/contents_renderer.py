...
def _bind_passages_with_ids(packet: dict[str, Any]) -> str:
    """Every admissible passage, in one canonical order with explicit delimiters.

    Ordering is by ``(source_id, passage_id)`` so the same packet always renders
    the same bytes regardless of the order the passages arrived in. Offsets are
    deliberately omitted: they are provenance the manifest already binds, and
    they would only spend input tokens.
    ...
    for passage in passages:
        ...
        text = _require_str(passage.get("text"), "passage text")
        ...
        rows.append((source_id, passage_id, published, text))

    seen = {(source_id, passage_id) for source_id, passage_id, _, _ in rows}
    ...
    blocks = [
        f"[passage_id: {passage_id}] [source_id: {source_id}] "
        f"[publication_date: {published}]\n{text}"
        for source_id, passage_id, published, text in sorted(rows)
    ]
