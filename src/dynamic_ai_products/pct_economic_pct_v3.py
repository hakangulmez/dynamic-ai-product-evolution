"""Validate economic-product PCT output with direct capability-to-task links."""
...
OUTPUT_CONTRACT = "pct_item1_economic_pct_output@0.3.0"
OUTPUT_SCHEMA = "schemas/pct_item1_economic_pct_output.v3.schema.json"
...
def validate_economic_pct_output_v3(
    raw: str,
    packet: dict[str, Any],
    discovery: dict[str, Any],
    validator: Draft202012Validator,
) -> dict[str, Any]:
    """Validate exact candidate accounting and product-local task capability links."""
    try:
    ...
    for product in parsed["economic_products"]:
        ...
        _check_refs([product, *product["capabilities"], *product["tasks"]], refs)
        capabilities = _unique(product["capabilities"], kind=f"{product['id']} capabilities")
        _unique(product["tasks"], kind=f"{product['id']} tasks")
