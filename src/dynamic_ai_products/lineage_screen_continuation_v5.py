"""Continuation with a bounded, visible truncated-output outcome (ADR-122).

The fifth stop was the first that was not a transport problem. One row's
``generateContent`` returned a well-formed envelope carrying exactly one
candidate with ``finishReason: MAX_TOKENS`` and 72,928 characters of unfinished
JSON. ADR-121's tolerance correctly did not apply: the transport worked, and a
truncated answer is a content outcome, not a provider one. The envelope rule
refused it, as it should, and the run stopped.

This successor adds the matching outcome for that case, and nothing else. One
change from ADR-121.

**A row the model never finished becomes a visible outcome.** When a
generation returns exactly one candidate with ``finishReason: MAX_TOKENS``, the
row is recorded as ``MODEL_OUTPUT_TRUNCATED`` and the run continues. It is not
a skip and not a weaker evidence rule: there is no screen JSON to judge,
because the answer stopped mid-sentence. The row keeps its identity, the closed
reason ``max_tokens``, its attempt telemetry and the reference and digest of
the captured envelope that proves it, so the evidence stays addressable. It
carries no status, no output, no archetypes, no evidence and no confidence, and
is excluded from classifier call lists and from every valid-status count.

**A truncated answer is never re-sent.** The model returned; repeating the
identical request invents nothing and costs a second generation. The row is
recorded from the capture already on disk.

**The first such row is re-derived, not re-called.** This successor admits a
source that stopped on a captured MAX_TOKENS envelope, and reconstructs that
row from the source's own hash-verified capture rather than spending a call on
it. Live sending resumes at the row after it.
...
The twenty-sixth truncated row stops the run fail-closed with no manifest,
because a model truncating that often is a systematic condition rather than a
stray long answer.
...
**The source loader admits one more shape, and inverts one proof.** A failed
continuation whose stop was a truncated answer is reusable, but the admission
is not "any ``provider_error``". Every earlier source shape proved itself by
what its stopping row did NOT capture; this one proves itself by what it DID.
The stopping row must hold a non-empty countTokens capture and a
generateContent capture whose bytes parse to exactly one candidate with
``finishReason: MAX_TOKENS``, and its counters must show exactly one count and
exactly one generate attempt, because a returned answer is never retried. A
source that stopped for another reason, whose stopping-row envelope is
malformed, blocked, multi-candidate or normally finished, or whose counters do
not close, is refused before a run directory exists.

ADR-120's "zero empty generate bodies anywhere in the run" proof is
deliberately not carried forward: since ADR-119 and ADR-120 an empty body is a
recovered, archived row like any other, and every reused row is revalidated
offline regardless. Requiring zero would refuse a sound source for a condition
its own prefix already answers.
...
every earlier loader refuses the directory. Promoting an ADR-122 run to a
...
__all__ = [
    ...
    "load_continuation_source_v5",
    "require_continuation_v5_run",
    "run_lineage_screen_continuation_v5",
]

AUTHORIZATION_SCHEMA = "schemas/universe_screen_continuation_authorization.v5.schema.json"
MANIFEST_SCHEMA = "schemas/universe_screen_continuation_manifest.v5.schema.json"
RECORD_SCHEMA = "schemas/universe_screen_record.v5.schema.json"
...
#: The separately governed truncated-output threshold.
MAX_MODEL_OUTPUT_TRUNCATED = 25

#: The closed reason and the one finish state that produces it.
...
CONTINUATION_V5_MANIFEST_FILENAME = "universe_screen_continuation_v5_manifest.json"
CONTINUATION_V5_RECORDS_FILENAME = "universe_screen_continuation_v5_records.jsonl"
...
#: This route continues a run that stopped on a captured, unfinished answer.
#: The reason string is a necessary condition only; the sufficient one is the
#: stopping row's own capture, read and re-hashed below.
_ACCEPTED_SOURCE_REASONS = ("provider_error",)


@dataclass
class TruncatedSourcePrefix(SourcePrefix):
    """A source prefix plus the truncation its stopping row already proved."""

    stopping_truncation: dict = field(default_factory=dict)
...
def load_continuation_source_v5(
    ...
    for filename, what in ((SCREEN_MANIFEST_FILENAME, "authoritative manifest"),
                           (CONTINUATION_V5_MANIFEST_FILENAME, "continuation manifest"),
                           (CONTINUATION_V5_RECORDS_FILENAME, "continuation records")):
    ...
    if reason not in _ACCEPTED_SOURCE_REASONS:
        raise ScreenInputError(
            f"The source stopped with reason {reason!r} ({receipt['detail']!r}). "
            "This route continues a truncated-output stop only; any other "
            "failure needs its own design and its own tests."
        )
    # The stop's own account must name the finish reason its capture shows;
    # the reason code alone is shared with every other provider failure.
    if TRUNCATED_FINISH_REASON not in receipt["detail"]:
        raise ScreenInputError(
            f"The source's terminal detail {receipt['detail']!r} does not name "
            f"{TRUNCATED_FINISH_REASON}; this route continues a "
            "truncated-output stop only."
        )
    ...
    # (5) The capture evidence that identifies a truncated-output stop. Unlike
    # every earlier source shape, this one REQUIRES the stopping row's
    # captures: the model answered, and that answer is the evidence.
    capture_dir = _capture_dir_for(directory, receipt)
    if not capture_dir.is_dir():
        raise ScreenInputError(
            "The source's stopping row has no capture directory; a truncated "
            "answer is proven by its own captured envelope, so a row that "
            "captured nothing failed for some other reason."
        )
    counts = sorted(capture_dir.glob("count-attempt-*.bin"))
    generates = sorted(capture_dir.glob("generate-attempt-*.bin"))
    if not counts or not any(x.stat().st_size > 0 for x in counts):
        raise ScreenInputError(
            "The source's stopping row carries no non-empty countTokens "
            "capture; a truncated answer always measured its input first."
        )
    if not generates:
        raise ScreenInputError(
            "The source's stopping row persisted no generateContent capture; "
            "there is no answer to read, so this is a different failure."
        )
    generate_bytes = generates[-1].read_bytes()
    truncated = _truncated_envelope(generate_bytes)
    if truncated is None:
        raise ScreenInputError(
            "The source's stopping-row envelope is not a single-candidate "
            f"{TRUNCATED_FINISH_REASON} response; a malformed, blocked, "
            "multi-candidate or normally finished envelope is not a "
            "truncation and is not continuable here."
        )
    stopping_capture = {
        "reason_code": TRUNCATED_REASON,
        "finish_reason": truncated["finish_reason"],
        "capture_reference": str(generates[-1].relative_to(directory)),
        "capture_sha256": _sha256(generate_bytes),
        "count_attempts": len(counts),
        "generate_attempts": len(generates),
        "candidate_token_count": truncated["candidate_token_count"],
    }
    ...
    # (8) The arithmetic: the stopping row measured once and generated once,
    # because a returned answer is never retried.
    stopping_generates = receipt["provider_attempts_made"] - suffix_done
    stopping_counts = receipt["count_attempts_made"] - suffix_done
    if stopping_generates != 1:
        raise ScreenInputError(
            f"The source's stopping row spent {stopping_generates} generate "
            "attempt(s); a truncated answer is returned once and never "
            "retried, so this row failed elsewhere."
        )
    if stopping_counts != 1:
        raise ScreenInputError(
            f"The source's stopping row spent {stopping_counts} countTokens "
            "attempt(s); this terminal shape spends exactly one."
        )
    return TruncatedSourcePrefix(
        run_dir=directory,
        ...
        entries=entries,
        stopping_truncation=stopping_capture,
    )


def require_continuation_v5_run(run_dir: str | Path) -> Path:
    ...
    manifest_path = directory / CONTINUATION_V5_MANIFEST_FILENAME
    if not manifest_path.is_file():
        raise ScreenInputError(
            f"Continuation run {directory} has no {CONTINUATION_V5_MANIFEST_FILENAME}; "
...
def _truncated_envelope(raw: bytes) -> dict | None:
    """Read one captured envelope and decide whether it is a truncation.

    Structural, never textual: the bytes must parse as Vertex transport JSON,
    carry exactly one candidate, and that candidate's ``finishReason`` must be
    ``MAX_TOKENS``. A blocked envelope, a malformed one, a multi-candidate one
    and a normally finished one all return ``None`` and keep their existing
    meanings.
    """
    try:
    ...
    if not isinstance(candidate, dict):
        return None
    if candidate.get("finishReason") != TRUNCATED_FINISH_REASON:
        return None
    usage = document.get("usageMetadata")
    tokens = usage.get("candidatesTokenCount") if isinstance(usage, dict) else None
    if isinstance(tokens, bool) or not isinstance(tokens, int):
        tokens = None
    return {"finish_reason": TRUNCATED_FINISH_REASON, "candidate_token_count": tokens}


def _classify_model_output_truncated(
    run_dir: Path, spent: list[dict]
) -> dict | None:
    """Decide whether one failed row's terminal capture is a truncation.

    The decision is read from the persisted bytes, which are re-hashed against
    the ledger line that claims them, so a capture that drifted on disk cannot
    be accepted as evidence of anything.
    """
    generates = [e for e in spent if e["operation_label"] == "generate_content"]
    if not generates:
        return None
    terminal = generates[-1]
    if terminal["capture_disposition"] != "raw_persisted" or not terminal.get("raw_reference"):
        return None
    ...
    return {
        ...
        "capture_reference": terminal["raw_reference"],
        "capture_sha256": terminal["raw_sha256"],
        "count_attempts": len(counts),
        "generate_attempts": len(generates),
        "candidate_token_count": envelope["candidate_token_count"],
    }
...
def _preflight(
    *,
    ...
    authorization, _ = _hydrate_pinned(
        ...
        "screen continuation v5 authorization",
    )
    ...
    prefix = load_continuation_source_v5(
    ...
    # The stopping row is re-derived from the capture the source already took.
    # It is never re-sent, so live sending resumes at the row after it.
    truncated_packet = suffix[0]
    suffix = suffix[1:]
    if not suffix:
        raise ScreenInputError(
            "Only the truncated stopping row would remain; there is nothing "
            "left to send and no continuation to run."
        )
    # "Reused" covers every row taken from the source: the archived prefix
    # plus the one row re-derived from its capture.
    reused = len(prefix_records) + 1
    called, cohort = len(suffix), len(packets)
    if (
        ...
        raise ScreenInputError(
            ...
            f"derive {cohort} / {reused} / {called} (the reused count includes "
            "the one row re-derived from the source's capture)."
        )
    ...
    if authorization["max_model_output_truncated"] != MAX_MODEL_OUTPUT_TRUNCATED:
        raise ScreenInputError(
            "The authorization does not pin this route's truncated-output "
            f"threshold of {MAX_MODEL_OUTPUT_TRUNCATED}."
        )
    ...
    return _Preflight(
        ...
        truncated_packet, model_route,
    )


def run_lineage_screen_continuation_v5(
    *,
    repo_root: str | Path,
    packet_manifest_path: str | Path,
    selection_artifact_path: str | Path,
    ...
    if dry_run:
        ...
        return ScreenRunResult(
            ...
            request_accounting={
                ...
                "reused_prefix_rows": len(pre.prefix_records) + 1,
    ...
    # the archived prefix plus the one re-derived stopping row
    adapter._row_ordinal = len(pre.prefix_records) + 1
    ...
    # The source's stopping row, reconstructed from the capture it already
    # took. No send is made for it, and it never enters the raw archive: a
    # truncated answer is not a usable model response.
    _derived_rendered, _ = render_diagnostic_prompt_with_citation_refs(
        pre.prompt_text, pre.truncated_packet)
    derived_truncated = {
        **base_identity(pre.truncated_packet, _derived_rendered),
        "record_kind": "model_output_truncated",
        "screen_status": None,
        "screen_output": None,
        "failure_reason_code": TRUNCATED_REASON,
        "failure_detail": _detail(
            "The model did not finish this answer: the source run captured a "
            f"single candidate with finishReason {TRUNCATED_FINISH_REASON}. "
            "Re-derived from that capture rather than re-sent."),
        "truncation_evidence": dict(pre.prefix.stopping_truncation),
        "row_provenance": {
    ...
    truncated_rows.append(derived_truncated)
    records.append(derived_truncated)
    ...
    def fail(reason: str, detail: str, packet: dict, *,
        ...
        receipt = {
            ...
            "max_model_output_truncated": MAX_MODEL_OUTPUT_TRUNCATED,
            "count_attempt_cap": pre.authorization["count_attempt_cap"],
            "provider_attempt_cap": pre.authorization["provider_attempt_cap"],
        ...
        try:
            write_bytes_once(
                ...
                what="continuation v5 failure receipt")
    ...
    for packet in pre.suffix:
        ...
        try:
            ...
            truncation = _classify_model_output_truncated(run_dir, spent)
            if truncation is not None:
                if len(truncated_rows) + 1 > MAX_MODEL_OUTPUT_TRUNCATED:
                    return fail(
                        "model_output_truncated_budget_exhausted",
                        "A twenty-sixth row returned an unfinished answer; the "
                        f"authorized tolerance is {MAX_MODEL_OUTPUT_TRUNCATED}.",
                        packet)
                truncated_row = {
    ...
    truncated_reused = [r for r in truncated
    ...
    counts = {
        ...
        "cohort_rows": len(pre.packets),
        # reused = the archived prefix + the re-derived stopping row; the
        # split is readable as model_output_truncated_reused_from_source.
        ...
        "model_output_truncated": len(truncated),
        "model_output_truncated_reused_from_source": len(truncated_reused),
        "max_model_output_truncated": MAX_MODEL_OUTPUT_TRUNCATED,
    ...
    reconciliation = {
        "records partition the retained rows across all five populations": (
            len(records) == counts["planned_rows"]
            == len(screened) + len(unverified) + len(insufficient)
            + len(unresolved) + len(truncated)),
        "truncated rows stayed within the authorized tolerance": (
            len(truncated) <= MAX_MODEL_OUTPUT_TRUNCATED
            == pre.authorization["max_model_output_truncated"]),
        "every truncated row names the closed reason and MAX_TOKENS": all(
            r["failure_reason_code"] == TRUNCATED_REASON
            and r["truncation_evidence"]["finish_reason"] == TRUNCATED_FINISH_REASON
            for r in truncated),
        "no truncated row carries a status, output or archived response": all(
            r["screen_status"] is None and r["screen_output"] is None
            and r["raw_response_id"] is None and r["raw_response_sha256"] is None
            for r in truncated),
        "every truncated row cites a captured envelope by digest": all(
            isinstance(r["truncation_evidence"], dict)
            and len(r["truncation_evidence"]["capture_sha256"]) == 64
            and r["truncation_evidence"]["capture_reference"]
            for r in truncated),
        "no other row kind claims truncation evidence": all(
            r["truncation_evidence"] is None for r in records
            if r["record_kind"] != "model_output_truncated"),
        "the source's stopping row was re-derived, never re-sent": (
            len(truncated_reused) == 1
            and truncated_reused[0]["truncation_evidence"]["capture_sha256"]
            == pre.prefix.stopping_truncation["capture_sha256"]
            and (truncated_reused[0]["cik"], truncated_reused[0]["accession"])
            == (pre.prefix.receipt["stopping_cik"],
                pre.prefix.receipt["stopping_accession"])),
        "the archive holds no line for any truncated row": not (
            {(r["cik"], r["accession"]) for r in truncated}
        ...
        "reused and called rows partition the cohort": (
            len(reused) == len(pre.prefix_records) + len(truncated_reused)
        ...
        "the reused prefix and derived row cost this run no send": all(
            e["row_ordinal"] > len(pre.prefix_records) + 1 for e in ledger),
        "the source's stopping row was never re-sent": (
            (pre.prefix.receipt["stopping_cik"],
             pre.prefix.receipt["stopping_accession"])
            not in {(r["cik"], r["accession"]) for r in called}),
        "the first sent row follows the source's stopping row": (
            not called or (called[0]["cik"], called[0]["accession"])
            == (pre.suffix[0]["cik"], pre.suffix[0]["accession"])),
        ...
        "the archive holds one line per resolved model-called row": (
            ...
            - len(unresolved) - (len(truncated) - len(truncated_reused))),
    ...
    if not all(reconciliation.values()):
        raise ScreenInputError(
            "Continuation v5 reconciliation failed; no records JSONL, no capture "
    ...
    try:
        write_bytes_once(run_dir / CONTINUATION_V5_RECORDS_FILENAME, records_bytes,
                         what="continuation v5 records")
        write_bytes_once(run_dir / CAPTURE_LEDGER_FILENAME, ledger_bytes,
                         what="continuation v5 capture ledger")
    except WriteOnceError as exc:
        raise ScreenInputError(str(exc)) from exc
    request_accounting = {
    ...
    manifest = {
        ...
        "truncated_output_policy": {
            "max_model_output_truncated": MAX_MODEL_OUTPUT_TRUNCATED,
            "closed_reason": TRUNCATED_REASON,
        ...
        "output_hashes": {
            CONTINUATION_V5_RECORDS_FILENAME: _sha256(records_bytes),
            RAW_RESPONSES_FILENAME: _sha256(archive_raw),
            CAPTURE_LEDGER_FILENAME: _sha256(ledger_bytes),
        },
        "run_timestamp": clock().isoformat(),
        "schema_versions": {
            "universe_screen_record_v5": "0.5.0",
            "universe_screen_continuation_manifest_v5": "0.12.0",
            "universe_screen_continuation_authorization_v5": "0.5.0",
        ...
        "limitations": [
            ...
            "result or exclusion.",
            "MODEL_OUTPUT_TRUNCATED means the model returned a well-formed "
            "envelope it did not finish (finishReason MAX_TOKENS). The "
            "transport worked and no screen JSON exists to judge, so the row "
            "carries no status, no evidence and no archived response, only the "
            "digest of the captured envelope. Such an answer is never re-sent, "
            "and the first row of this lineage was re-derived from its source "
            "run's capture rather than re-called.",
    ...
    _validate(manifest, _load_schema(root, MANIFEST_SCHEMA), "Continuation v5 manifest")
    try:
        write_bytes_once(
            run_dir / CONTINUATION_V5_MANIFEST_FILENAME,
            (json.dumps(manifest, indent=2, sort_keys=True) + "\n").encode("utf-8"),
            what="continuation v5 manifest")
    except WriteOnceError as exc:
        raise ScreenInputError(str(exc)) from exc
    ...
    result.manifest_path = run_dir / CONTINUATION_V5_MANIFEST_FILENAME
    return result
