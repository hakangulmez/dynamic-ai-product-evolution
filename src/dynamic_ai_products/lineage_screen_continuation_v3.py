"""Continuation from a run that stopped on an empty count body (ADR-120).

ADR-119's continuation reused 4,297 rows and added 524 more before
``countTokens`` returned once with no usable body and stopped it. The bounded
count retry never engaged, because a call that returns raises nothing — the
same structural gap ADR-119 closed for the generation, on the operation it
deliberately left out of scope. That directory now holds 4,821 rows of
finished work and no way to become anything.

Two changes from ADR-119, and no others.

**The connector answers an empty count body.** It retries that one anomaly
through the unchanged ADR-118 count schedule, and inherits ADR-119's generate
behaviour by import rather than restatement. An empty count never invokes the
generation for that attempt: the measurement is a precondition.
...
__all__ = [
    "CONTINUATION_V3_MANIFEST_FILENAME",
    "CONTINUATION_V3_RECORDS_FILENAME",
    "EMPTY_COUNT_TERMINAL_REASON",
    "load_continuation_source_v3",
    "require_continuation_v3_run",
    "run_lineage_screen_continuation_v3",
]

AUTHORIZATION_SCHEMA = "schemas/universe_screen_continuation_authorization.v3.schema.json"
MANIFEST_SCHEMA = "schemas/universe_screen_continuation_manifest.v3.schema.json"
...
MANIFEST_CONTRACT = "universe_screen_continuation_manifest@0.10.0"
...
CONTINUATION_V3_MANIFEST_FILENAME = "universe_screen_continuation_v3_manifest.json"
CONTINUATION_V3_RECORDS_FILENAME = "universe_screen_continuation_v3_records.jsonl"
...
def load_continuation_source_v3(
    ...
    for filename, what in ((SCREEN_MANIFEST_FILENAME, "authoritative manifest"),
                           (CONTINUATION_V3_MANIFEST_FILENAME, "continuation manifest"),
                           (CONTINUATION_V3_RECORDS_FILENAME, "continuation records")):
...
def require_continuation_v3_run(run_dir: str | Path) -> Path:
    ...
    manifest_path = directory / CONTINUATION_V3_MANIFEST_FILENAME
    if not manifest_path.is_file():
        raise ScreenInputError(
            f"Continuation run {directory} has no {CONTINUATION_V3_MANIFEST_FILENAME}; "
...
def _preflight(
    *,
    ...
    authorization, _ = _hydrate_pinned(
        ...
        "screen continuation v3 authorization",
    )
...
def run_lineage_screen_continuation_v3(
    *,
    repo_root: str | Path,
    packet_manifest_path: str | Path,
    selection_artifact_path: str | Path,
    ...
    def fail(reason: str, detail: str, packet: dict, *,
        ...
        try:
            write_bytes_once(
                ...
                what="continuation v3 failure receipt")
    ...
    for packet in pre.suffix:
        ...
        try:
            ...
            # A row whose every permitted attempt of one operation returned
            # nothing gets its own truthful terminal reason; the closed
            # provider enum cannot say either of them.
            if empty_counts and empty_counts == row_counts and row_generates == 0:
                rows_with_empty_count += 1
                return fail(EMPTY_COUNT_TERMINAL_REASON,
                            f"Every permitted countTokens attempt returned an "
                            f"empty body ({empty_counts} attempts); the "
                            "generation was never invoked.", packet)
            if empties and empties == row_generates:
                rows_with_empty_body += 1
                return fail(EMPTY_GENERATE_TERMINAL_REASON,
    ...
    if not all(reconciliation.values()):
        raise ScreenInputError(
            "Continuation v3 reconciliation failed; no records JSONL, no capture "
    ...
    try:
        write_bytes_once(run_dir / CONTINUATION_V3_RECORDS_FILENAME, records_bytes,
                         what="continuation v3 records")
        write_bytes_once(run_dir / CAPTURE_LEDGER_FILENAME, ledger_bytes,
                         what="continuation v3 capture ledger")
    except WriteOnceError as exc:
        raise ScreenInputError(str(exc)) from exc
    request_accounting = {
    ...
    manifest = {
        ...
        "output_hashes": {
            CONTINUATION_V3_RECORDS_FILENAME: _sha256(records_bytes),
        ...
        "schema_versions": {
            ...
            "universe_screen_continuation_manifest_v3": "0.10.0",
            "universe_screen_continuation_authorization_v3": "0.3.0",
    ...
    _validate(manifest, _load_schema(root, MANIFEST_SCHEMA), "Continuation v3 manifest")
    try:
        write_bytes_once(
            run_dir / CONTINUATION_V3_MANIFEST_FILENAME,
            (json.dumps(manifest, indent=2, sort_keys=True) + "\n").encode("utf-8"),
            what="continuation v3 manifest")
    except WriteOnceError as exc:
        raise ScreenInputError(str(exc)) from exc
    ...
    result.manifest_path = run_dir / CONTINUATION_V3_MANIFEST_FILENAME
    return result
