"""Continuation from a run that stopped on an empty generate body (ADR-119).

The ADR-118 continuation reused 3,939 rows and added 358 more before a
``generateContent`` call returned an empty entity body and stopped it. That
directory now holds 4,297 rows of finished work and, like its parent, no way
to become anything. This successor gives it one, on the same terms as before:
**reuse is earned, never assumed.**

Two changes from ADR-118, and no others.

**The connector answers an empty body.** ADR-119's connector retries that one
anomaly through the unchanged ADR-117 schedule, so the failure this source ran
into is now survivable rather than terminal on first occurrence.

**The source loader admits one more shape, narrowly.** A failed continuation
whose stop was an empty generate body is reusable — but the admission is not
"any ``provider_response_unusable``". Seven independent proofs must hold, and
the decisive ones are read from the source's own captures rather than from its
...
stopped for a different reason, that persisted a generate body it could not
use, or whose counters do not close, is refused before a run directory exists.
...
__all__ = [
    "CONTINUATION_V2_MANIFEST_FILENAME",
    "CONTINUATION_V2_RECORDS_FILENAME",
    "EMPTY_BODY_TERMINAL_REASON",
    "load_continuation_source_v2",
    "require_continuation_v2_run",
    "run_lineage_screen_continuation_v2",
]

AUTHORIZATION_SCHEMA = "schemas/universe_screen_continuation_authorization.v2.schema.json"
MANIFEST_SCHEMA = "schemas/universe_screen_continuation_manifest.v2.schema.json"
...
MANIFEST_CONTRACT = "universe_screen_continuation_manifest@0.9.0"
...
CONTINUATION_V2_MANIFEST_FILENAME = "universe_screen_continuation_v2_manifest.json"
CONTINUATION_V2_RECORDS_FILENAME = "universe_screen_continuation_v2_records.jsonl"
...
EMPTY_BODY_TERMINAL_REASON = "empty_generate_body_exhausted"
...
_ACCEPTED_SOURCE_REASONS = ("provider_error", EMPTY_BODY_TERMINAL_REASON)
...
def load_continuation_source_v2(
    ...
    for filename, what in ((SCREEN_MANIFEST_FILENAME, "authoritative manifest"),
                           (CONTINUATION_V2_MANIFEST_FILENAME, "continuation manifest"),
                           (CONTINUATION_V2_RECORDS_FILENAME, "continuation records")):
    ...
    # (5)/(6) The capture evidence that actually identifies an empty-body stop.
    capture_dir = _capture_dir_for(directory, receipt)
    count_bodies = sorted(capture_dir.glob("count-attempt-*.bin")) if capture_dir.is_dir() else []
    if not count_bodies or not any(p.stat().st_size > 0 for p in count_bodies):
        raise ScreenInputError(
            "The source's stopping row carries no non-empty countTokens "
            "capture; an empty-generate-body stop always measured its input "
            "first, so this is a different failure."
        )
    generate_bodies = sorted(capture_dir.glob("generate-attempt-*.bin"))
    if generate_bodies:
        raise ScreenInputError(
            f"The source's stopping row persisted {len(generate_bodies)} "
            "generate body/bodies; an empty-body stop persists none, so this "
            "row failed for some other reason and is not continuable here."
        )
...
def require_continuation_v2_run(run_dir: str | Path) -> Path:
    ...
    manifest_path = directory / CONTINUATION_V2_MANIFEST_FILENAME
    if not manifest_path.is_file():
        raise ScreenInputError(
            f"Continuation run {directory} has no {CONTINUATION_V2_MANIFEST_FILENAME}; "
...
def _preflight(
    *,
    ...
    authorization, _ = _hydrate_pinned(
        ...
        "screen continuation v2 authorization",
    )
    ...
    prefix = load_continuation_source_v2(
...
def run_lineage_screen_continuation_v2(
    *,
    repo_root: str | Path,
    packet_manifest_path: str | Path,
    selection_artifact_path: str | Path,
    ...
    connector = VertexGeminiScreenV5(
    ...
    def fail(reason: str, detail: str, packet: dict, *,
        ...
        try:
            write_bytes_once(
                ...
                what="continuation v2 failure receipt")
    ...
    for packet in pre.suffix:
        ...
        try:
            ...
            # A row whose every permitted attempt returned nothing gets its own
            # truthful terminal reason; the closed provider enum cannot say it.
            if empties and empties == sum(
                e["operation_label"] == "generate_content" for e in spent
            ):
                rows_with_empty_body += 1
                return fail(EMPTY_BODY_TERMINAL_REASON,
                            f"Every permitted generateContent attempt returned an "
                            f"empty body ({empties} attempts).", packet)
    ...
    reconciliation = {
        ...
        "reused and called rows partition the cohort": (
            len(reused) == len(pre.prefix_records) and len(called) == len(pre.suffix)),
        ...
        "empty-body attempts are counted but never archived": (
            ...
            and len(archive_entries) == len(pre.prefix_records) + len(called)),
    ...
    if not all(reconciliation.values()):
        raise ScreenInputError(
            "Continuation v2 reconciliation failed; no records JSONL, no capture "
    ...
    try:
        write_bytes_once(run_dir / CONTINUATION_V2_RECORDS_FILENAME, records_bytes,
                         what="continuation v2 records")
        write_bytes_once(run_dir / CAPTURE_LEDGER_FILENAME, ledger_bytes,
                         what="continuation v2 capture ledger")
    except WriteOnceError as exc:
        raise ScreenInputError(str(exc)) from exc
    request_accounting = {
    ...
    manifest = {
        ...
        "provider": {"name": adapter.name, "connector": SCREEN_CONNECTOR_V5_ID,
                     "model_route": dict(adapter.model_route)},
        ...
        "output_hashes": {
            CONTINUATION_V2_RECORDS_FILENAME: _sha256(records_bytes),
        ...
        "schema_versions": {
            ...
            "universe_screen_continuation_manifest_v2": "0.9.0",
            "universe_screen_continuation_authorization_v2": "0.2.0",
    ...
    _validate(manifest, _load_schema(root, MANIFEST_SCHEMA), "Continuation v2 manifest")
    try:
        write_bytes_once(
            run_dir / CONTINUATION_V2_MANIFEST_FILENAME,
            (json.dumps(manifest, indent=2, sort_keys=True) + "\n").encode("utf-8"),
            what="continuation v2 manifest")
    except WriteOnceError as exc:
        raise ScreenInputError(str(exc)) from exc
    ...
    result.manifest_path = run_dir / CONTINUATION_V2_MANIFEST_FILENAME
    return result
