"""Five-firm smoke for economic PCT V3 with direct capability-to-task output."""
...
from .pct_economic_pct_v3 import (
    OUTPUT_CONTRACT,
    OUTPUT_SCHEMA,
...
PROMPT_PATH = "prompts/extraction/pct_item1_economic_pct_v3.md"
RUN_KIND = "pct_item1_economic_pct_smoke_v3"
RAW_FILENAME = "pct_item1_economic_pct_v3_raw_responses.jsonl"
RECORDS_FILENAME = "pct_item1_economic_pct_v3_records.jsonl"
MANIFEST_FILENAME = "pct_item1_economic_pct_v3_manifest.json"
HTML_FILENAME = "pct_item1_economic_pct_v3_human_review.html"
...
def _html(records: list[dict[str, Any]], run_id: str) -> str:
    ...
    chunks = [
        ...
        "<title>Economic PCT V3 review</title>",
        ...
        f"<h1>Economic PCT V3 smoke: {esc(run_id)}</h1>",
        "<p>Development-only Item 1 smoke. It maps economic products, capabilities, and customer tasks; it settles no score, tier, transformation depth, or universe membership.</p>",
    ]
    for record in records:
        ...
        for product in record["snapshot"]["economic_products"]:
            ...
            chunks.append("</ul><h4>Tasks</h4><ul>")
...
def run_economic_pct_v3_smoke(
    *,
    ...
    """Run the direct product-capability-task successor on saved discovery maps."""
    if dry_run:
        return {
            "run_id": run_id,
            "dry_run": True,
    ...
    if generate is None:
        raise ValueError("A live economic PCT V3 smoke requires generate.")
    ...
    for row in plan:
        ...
        try:
            snapshot = validate_economic_pct_output_v3(
    ...
    try:
        for filename, payload in (
            ...
            write_bytes_once(run_dir / filename, payload, what=f"economic PCT V3 {filename}")
