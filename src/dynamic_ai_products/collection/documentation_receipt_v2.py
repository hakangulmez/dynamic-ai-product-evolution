"""Documentation collection receipt v0.2.0: schema, loader and builder (ADR-038, E-C-D1).

**Why a successor rather than a repair.** ``documentation_collection_receipt@0.1.0``
is committed and already instantiated by a live receipt, and its ``failed`` branch
pins every payload field to ``null`` at *both* the schema and builder layers. A
collector that recorded what it actually observed could not publish under it at
all. The defect is therefore contractual, not procedural, and 0.1.0 is never
modified: it stays byte-frozen so the existing receipt remains verifiable against
the exact contract that produced it.

**What 0.2.0 adds.** A failed entry now carries the facts that were genuinely
established before the refusal -- the request-start instant, the observed HTTP
status, the adapter-exposed ``Location`` string, and the partial request chain --
plus a ``failure_phase`` naming exactly how far the entry got.
...
__all__ = [
    ...
    "RECEIPT_CONTRACT_V2",
    "RECEIPT_PROPERTIES_V2",
    "RECEIPT_REQUIRED_V2",
    "RECEIPT_SCHEMA_ID_V2",
    "REASON_PHASES",
    "RESPONSE_DISPOSITIONS",
    "SCHEMA_VERSION_V2",
    "build_documentation_receipt_v2",
    "classify_observed_location",
    "expected_receipt_schema_v2",
    "receipt_bytes_v2",
    "transcribable_location",
    "validate_receipt_schema_v2_bytes",
]

RECEIPT_CONTRACT_V2 = "documentation_collection_receipt@0.2.0"
RECEIPT_SCHEMA_ID_V2 = "documentation_collection_receipt.v2.schema.json"
...
REDIRECT_STATUSES_V2: tuple[int, ...] = (301, 308)
TERMINAL_STATUS_V2 = 200
_STATUS_MIN = 100
_STATUS_MAX = 599
...
RECEIPT_REQUIRED_V2: frozenset[str] = frozenset(
    {
...
RECEIPT_PROPERTIES_V2: frozenset[str] = RECEIPT_REQUIRED_V2
...
ENTRY_REQUIRED_V2: frozenset[str] = frozenset(
    {
...
ENTRY_PROPERTIES_V2: frozenset[str] = ENTRY_REQUIRED_V2
...
def _redirect_accepted(final_url: str) -> dict[str, Any]:
    ...
    return {
        "redirect_observed_status": {"enum": list(REDIRECT_STATUSES_V2)},
...
def _phase_rules(requested: str, final: str) -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        "persistence": {
            ...
            **dated,
            **_redirect_accepted(final),
            "terminal_observed_status": {"const": TERMINAL_STATUS_V2},
            "terminal_observed_location_disposition": {
                "enum": list(RESPONSE_DISPOSITIONS)
            },
...
def _entry_schema_v2(entry: dict[str, str]) -> dict[str, Any]:
    ...
    return {
        ...
        "required": sorted(ENTRY_REQUIRED_V2),
        "properties": {
...
def expected_receipt_schema_v2() -> dict[str, Any]:
    """The complete locked 0.2.0 schema. The committed file is generated from this."""
    return {
        "$schema": SCHEMA_DIALECT,
        "$id": RECEIPT_SCHEMA_ID_V2,
        "title": RECEIPT_CONTRACT_V2,
        "comment": (
            "ADR-038 (E-C-D1). Successor to documentation_collection_receipt@0.1.0, "
            "which is never modified because a live receipt already instantiates it. "
            "A failed entry now records what was actually established before the "
            "refusal -- failure_phase, the request-start timestamp, the observed HTTP "
            "status, and the adapter-exposed Location string -- instead of being "
            "blanked. request_chain replaces redirect_chain and lists only URLs this "
        ...
        "type": "object",
        "additionalProperties": False,
        "required": sorted(RECEIPT_REQUIRED_V2),
        "properties": {
            "contract": {"const": RECEIPT_CONTRACT_V2},
            ...
            "entries": {
                ...
                "minItems": 3,
                "maxItems": 3,
                "prefixItems": [_entry_schema_v2(e) for e in FROZEN_ENTRY_IDENTITIES],
                "items": False,
            },
...
def validate_receipt_schema_v2_bytes(schema_bytes: Any) -> str:
    """Prove the bytes are the locked 0.2.0 schema, semantically, and return the digest.

    Same closure as the 0.1.0 loader: a recursive JSON-type-exact comparison
    against :func:`expected_receipt_schema_v2`, duplicate member names refused at
    ...
    if isinstance(properties, dict):
        ...
        if isinstance(contract, dict):
            ...
            if (
                ...
                and declared.strip()
                and declared != RECEIPT_CONTRACT_V2
            ):
                _refuse(
    ...
    if not _json_exact_equal(schema, expected_receipt_schema_v2()):
...
def _entry_violations_v2(entry: Any, frozen: dict[str, str]) -> str | None:
    ...
    """
    if not isinstance(entry, dict) or frozenset(entry) != ENTRY_PROPERTIES_V2:
...
def _succeeded_violations(entry: dict[str, Any], requested: str, final: str) -> str | None:
    ...
    if entry.get("redirect_observed_status") not in REDIRECT_STATUSES_V2:
    ...
    if entry.get("terminal_observed_status") != TERMINAL_STATUS_V2:
...
def build_documentation_receipt_v2(
    *,
    attempt_id: str,
    code_commit: str,
    run_created_at: str,
    ...
    """Assemble the 0.2.0 receipt, refusing anything the committed schema would reject.
    ...
    for entry, frozen in zip(entries, FROZEN_ENTRY_IDENTITIES):
        violation = _entry_violations_v2(entry, frozen)
...
def receipt_bytes_v2(receipt: dict[str, Any]) -> bytes:
