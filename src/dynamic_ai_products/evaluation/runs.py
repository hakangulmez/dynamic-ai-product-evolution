...
from .models import CaseSetManifest, EvaluationRunManifest, model_contract_hash
...
def _serialize_manifest(manifest: EvaluationRunManifest, eval_run_id: str) -> bytes:
    try:
...
def load_evaluation_run_manifest(
    ...
    """Load and validate one run manifest from its fixed run-directory location.

    Symlink run directories and manifest files are rejected. The loader
    verifies the generated contract stamp and that the manifest's own
    ``eval_run_id`` equals the requested one; it infers no execution status
    and verifies no metric or result artifacts.
    """
    resolved_eval_root = _validate_eval_root(eval_root, "eval_root")
    run_id = _validate_run_id(eval_run_id)
    run_dir = resolved_eval_root / run_id
