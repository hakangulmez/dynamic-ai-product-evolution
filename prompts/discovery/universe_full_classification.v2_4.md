# Company-Universe Full Multi-Axis Classification — v2.4
...
**A quote is a copy operation, not a writing task.** You are not composing a
sentence that describes the passage; you are transcribing characters that are
already in it. Build every evidence object in exactly this order:

1. Decide the one narrow claim this object will support.
2. Locate a contiguous span, inside a single passage body, that proves that
   claim on its own.
3. Select that span — its start and end characters.
4. Copy it character for character into `quote`.
5. Only then copy the `P`-reference from that same passage's header into
   `passage_ref`.
6. Verify the copied quote occurs, exactly as copied, in that cited passage.

If step 6 fails, you have written rather than copied. Go back to step 3.

- Return only that `passage_ref`, exactly as displayed.
- `quote` is a contiguous verbatim substring from that one passage body.
- Never normalize, summarize, paraphrase, re-punctuate, expand an abbreviation,
  fix a typo, or tidy whitespace inside a quote. Copy the characters that are
  there, including any that look wrong.
- Never compose a quote from two sentences, two passages, or two separated
  spans, and never join spans with an ellipsis.
- Never truncate a span into something the claim no longer follows from.
- Never move a quote to a different `P`-reference. The reference is copied from
  the passage the span came from, never chosen afterwards.
- Do not return a source id, passage id, hidden hash, URL, or filing id.
- **If no exact span can be copied, omit that evidence object.** Set only the
  affected conclusion to unknown or null and add a concise boundary flag. An
  omitted object is correct; an approximated quote is not.
- Do not infer from a website, internal IT use, AI wording, a ticker,
  SIC/NAICS, or current company knowledge.
- A quote displayed in the admission context may be useful orientation, but
  reuse it only after locating and copying it from the complete supplied Item 1
  packet.

### A quote admits no character modification whatsoever

The span you copy is the span you return. Between locating it and emitting it,
nothing about it may change. Each of the following makes a quote invalid, and
the response carrying it is refused:

- **Ellipsis.** No `...`, no `…`, no `[...]`, no bracketed omission marker of
  any kind. An ellipsis inside a quote is a declaration that you skipped text,
  and a quote that skips text is not the span you found.
- **Splice.** Two spans that are each real do not become one real span by being
  written next to each other. If a claim needs two separated spans, it needs
  two evidence objects, or it needs a narrower claim.
- **Insertion.** No added word, article, connective, gloss, bracketed
  clarification, or leading subject. Do not prepend `Our`, `The Company`, `We`,
  or any other subject to a span that does not begin with it.
- **Deletion.** No dropped word, clause, parenthetical, footnote marker, or
  trailing qualifier, and no silent removal of a mid-span phrase.
- **Re-casing.** No change of an upper-case letter to lower case or of a lower
  case letter to upper case, at the start of the span or anywhere inside it. A
  span that begins mid-sentence begins with the character actually there, even
  when that character is lower case and the result reads oddly.
- **Any other character change.** Punctuation, spacing, hyphenation, quotation
  marks, digits, symbols, and accents are all copied exactly.

If the span you want begins awkwardly, extend it backwards to a natural
boundary that is really there, or choose a different span. Never repair it.
...
- Write the conclusion, not the argument for it. Nothing in this field should
  ...
  or "therefore".
- Do not restate what the quote already says. The quote is beside it; a reader
  has both.
...
- `quote`: at most 1200 characters, contiguous, from one passage body.
...
evidence. Within them, quote the shortest sufficient span and cite only the
...
- Every output bound holds: `customer_value_archetypes` at most 4;
  ...
  at most 1200 characters; each `supported_claim` at most 300; `boundary_flags`
  at most 4, each at most 160; `contradictions` at most 4, each at most 200.
- Every quote was copied, not written: each one occurs character for character
  inside the passage its `passage_ref` cites, and each `passage_ref` was copied
  from the passage the span came from.
- No quote was normalized, summarized, composed from two spans, truncated past
  its claim, or moved to another `P`-reference.
- No quote carries an ellipsis, a splice, an inserted word or leading subject,
  a deleted word or clause, a re-cased letter, or any other character
  modification: every quote is the located span, unaltered.
- Every quote is still sufficient for the claim it supports: no span was cut
  short to fit a limit.
...
- Any claim without an exact copyable span was dropped, its conclusion left
  unknown or null, and a boundary flag added.
