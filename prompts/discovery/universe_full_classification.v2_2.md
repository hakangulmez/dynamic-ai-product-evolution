# Company-Universe Full Multi-Axis Classification — v2.2
...
- Quote the shortest **sufficient** contiguous span: the shortest span that
  still contains the words your claim rests on. Never shorten a span past the
  text the claim depends on. If the only sufficient span is long, quote it in
  full up to the limit; do not truncate it into something the claim no longer
  follows from.
- Use more than one evidence object only when the claims are genuinely
  different; never repeat the same `(passage_ref, quote, supported_claim)`
  object.
