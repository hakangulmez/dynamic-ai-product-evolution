...
Apply a strict purchase-object test before returning `YES`: identify what the
external customer is contracting to obtain. Return `YES` only if that object is
the software product or software platform itself. If software merely enables,
distributes, manages, or gives access to another purchased good or service,
return `NO`. Terms such as digital, platform, cloud, technology, online, or
solution do not by themselves establish a software product.

Return `YES` only if Item 1 establishes that an external customer obtains the
software functionality itself as a separately identifiable product or platform,
rather than merely using it to access, sell, rent, pay for, or operate a
non-digital product or human-delivered service.
...
themselves customer-facing digital products. A managed service, consulting
engagement, implementation, integration, outsourcing, or staff-led operation is
not a digital product merely because it uses or manages software, cloud
infrastructure, data, or AI.
...
- `CORE` — use only when Item 1 establishes that, at the consolidated-firm
  level, customers principally acquire the software product or software
  platform itself. Do not use `CORE` merely because software delivers, enables,
  accompanies, or improves the firm's underlying customer service. If Item 1
  does not directly establish software itself as the firm's principal commercial
  offering, do not use `CORE`.
