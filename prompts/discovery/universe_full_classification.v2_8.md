# Company-Universe Full Multi-Axis Classification — v2.8
...
- Never repeat the same `(passage_ref, span_ref, span_interpretation)` object, and
...
### `span_interpretation` is a conclusion, not an explanation

`span_interpretation` states, as briefly as it can, **what the quoted span
...
of the passage, and not a restatement of the selected evidence text.
...
- `span_interpretation`: optional, and at most 300 characters when present.
  This is the one bound that is not contractual. If you have nothing to add
  beyond the span itself, omit the field or send null; do not invent a clause
  to fill it. An interpretation that is missing, empty or overlong is recorded
  as such and never discards your evidence -- but it must still be a string or
  null, never a number, list or object.
...
- Every non-unknown conclusion has a direct, resolving selected span.
...
- Every output bound holds: `customer_value_archetypes` at most 4;
  `complementary_dependencies` at most 5; `evidence` at most 12; each
  `span_ref` names a contiguous run inside one passage; each
  `span_interpretation`, when
  present, at most 300 and always a string or null;
  `boundary_flags`
...
- Every `span_interpretation` is a conclusion clause for its axis, not an
  explanatory sentence, not a summary of the passage, and not a restatement of
  the selected evidence text.
