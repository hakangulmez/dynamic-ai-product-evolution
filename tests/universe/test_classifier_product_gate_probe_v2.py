...
from dynamic_ai_products import lineage_classifier_product_gate_probe_v2 as probe
...
def _probe_grant(cohort, selection, tmp_path):
    ...
    authorization = {
        **base.authorization,
        "authorization_contract": probe.PRODUCT_GATE_PROBE_V2_ROUTE.authorization_contract,
        "authorization_id": "product-gate-probe-fixture",
        "run_kind": probe.PRODUCT_GATE_PROBE_V2_ROUTE.run_kind,
        "output_contract": probe.PRODUCT_GATE_PROBE_V2_ROUTE.record_contract,
        "output_axes_contract": probe.PRODUCT_GATE_PROBE_V2_ROUTE.axes_contract,
        "prompt_template_path": probe.PRODUCT_GATE_PROBE_V2_ROUTE.prompt_path,
        "prompt_template_sha256": _sha((ROOT / probe.PRODUCT_GATE_PROBE_V2_ROUTE.prompt_path).read_bytes()),
...
def test_prompt_adds_one_financial_delivery_boundary_without_altering_the_v7_prompt() -> None:
    text = (ROOT / probe.PRODUCT_GATE_PROBE_V2_ROUTE.prompt_path).read_text()
    ...
    assert "A financial product, payment service, account, loan, card, brokerage," in text
    assert "because customers access it digitally" in text
    assert "software platform itself is separately sold, licensed, or subscribed" in text
    assert "A financial product, payment service, account, loan, card, brokerage," not in (
        ROOT / "prompts/discovery/software_universe_classifier_pilot.v7.md"
    ).read_text()
...
def test_probe_contracts_are_well_formed_and_pin_only_the_new_prompt() -> None:
    for schema_path in (
        ...
        probe.PRODUCT_GATE_PROBE_V2_ROUTE.authorization_schema,
        probe.PRODUCT_GATE_PROBE_V2_ROUTE.manifest_schema,
    ):
        schema = json.loads((ROOT / schema_path).read_text())
        Draft202012Validator.check_schema(schema)
    auth = json.loads((ROOT / probe.PRODUCT_GATE_PROBE_V2_ROUTE.authorization_schema).read_text())
    manifest = json.loads((ROOT / probe.PRODUCT_GATE_PROBE_V2_ROUTE.manifest_schema).read_text())
    assert auth["properties"]["logical_row_cap"]["const"] == 5
    assert auth["properties"]["prompt_template_path"]["const"] == probe.PRODUCT_GATE_PROBE_V2_ROUTE.prompt_path
    assert manifest["properties"]["prompt_template_path"]["const"] == probe.PRODUCT_GATE_PROBE_V2_ROUTE.prompt_path
...
def test_probe_dry_run_uses_no_provider_and_writes_nothing(
    ...
    result = probe.run_product_gate_probe_v2(
        repo_root=ROOT,
        cohort_manifest_path=cohort.path,
...
def test_probe_fixture_run_preserves_five_rows_and_records_the_new_prompt(
    ...
    manifest = json.loads((result.run_dir / probe.PRODUCT_GATE_PROBE_V2_ROUTE.manifest_filename).read_text())
    assert manifest["counts"]["selected_rows"] == manifest["counts"]["classified"] == 5
    assert manifest["prompt_template_path"] == probe.PRODUCT_GATE_PROBE_V2_ROUTE.prompt_path
