...
def _prompt():
    return load_prompt(ROOT, "product_discovery_schema_v4")
...
def test_the_record_qualifies_the_prompt_the_plan_actually_resolves():
    ...
    """
    assert EXTRACTION_PROMPTS[STAGE][0] == "product_discovery_schema_v4"
...
@pytest.mark.parametrize(
    "version",
    # ADR-064 published v6, ADR-065 published v7, ADR-069 published v8 and
    # ADR-073 published v9, so the never-published probe moves with them. This
    # parameter is meant to name a version this code has never issued; leaving
    # a published one here would assert the opposite of what the test says.
    ["extraction_prompt_registry_v10", "extraction_prompt_registry", "", "v5", None, 2],
)
