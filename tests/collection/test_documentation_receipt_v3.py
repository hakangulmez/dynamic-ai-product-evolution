"""The documentation_collection_receipt@0.3.0 contract (ADR-038, E-C-D1).
...
The committed file is *generated* from :func:`expected_receipt_schema_v3`, so the
...
from dynamic_ai_products.collection.documentation_routes import (
    FROZEN_ROUTE_IDENTITIES as FROZEN_ENTRY_IDENTITIES,
)
from dynamic_ai_products.collection.documentation_receipt_v2 import (
    expected_receipt_schema_v2 as expected_prev,
)
from dynamic_ai_products.collection.documentation_receipt_v2 import (
    validate_receipt_schema_v2_bytes as validate_prev,
)
...
COMMITTED_VALIDATOR = Draft202012Validator(json.loads(V3_BYTES.decode("utf-8")))
...
def _build(entries: list[dict], completion: str) -> dict:
    return v3.build_documentation_receipt_v3(
        attempt_id=ATTEMPT,
        code_commit=COMMIT,
...
def test_the_committed_schema_is_exactly_the_constructor_output():
    assert V3_BYTES == _serialize(v3.expected_receipt_schema_v3())
    assert v3.validate_receipt_schema_v3_bytes(V3_BYTES) == sha256(V3_BYTES).hexdigest()


def test_the_schema_is_a_legal_2020_12_schema():
    Draft202012Validator.check_schema(v3.expected_receipt_schema_v3())


def test_the_contract_identity_is_the_successor():
    schema = v3.expected_receipt_schema_v3()
    assert schema["properties"]["contract"]["const"] == "documentation_collection_receipt@0.3.0"
    assert schema["properties"]["schema_version"]["const"] == "0.3.0"
    assert schema["$id"] == "documentation_collection_receipt.v3.schema.json"
...
def test_the_two_loaders_reject_each_other_s_file():
    with pytest.raises(CollectionError) as v3_on_v2:
        v3.validate_receipt_schema_v3_bytes(V2_BYTES)
    with pytest.raises(CollectionError) as v2_on_v3:
        validate_prev(V3_BYTES)
    assert v3_on_v2.value.reason_code == "receipt_schema_contract_mismatch"
    assert v2_on_v3.value.reason_code == "receipt_schema_contract_mismatch"


def test_both_earlier_contracts_are_untouched_by_this_increment():
    ...
    assert validate_prev(V2_BYTES) == sha256(V2_BYTES).hexdigest()
    assert expected_v1()["properties"]["contract"]["const"].endswith("@0.1.0")
    assert expected_prev()["properties"]["contract"]["const"].endswith("@0.2.0")
    # Their frozen routes are the ones they were built against, not v0.3's.
    historical = expected_v1()["properties"]["entries"]["prefixItems"][0]
    assert historical["properties"]["final_url"]["const"] != (
        FROZEN_ENTRY_IDENTITIES[0]["final_url"]
    )


# --- what 0.3.0 actually changes ---------------------------------------------


def test_the_v3_routes_come_from_the_routes_module_only():
    """One declaration for the whole v0.3 stack."""
    from dynamic_ai_products.collection import documentation_routes as routes

    assert FROZEN_ENTRY_IDENTITIES is routes.FROZEN_ROUTE_IDENTITIES
    assert routes.ROUTE_SET_VERSION == "0.3.0"
    source = (SRC / "documentation_receipt_v3.py").read_text(encoding="utf-8")
    assert '"https://' not in source, "the v0.3 contract declares no route itself"


def test_the_e1_route_is_the_observed_hypothesis_and_e2_e3_are_untouched():
    """ADR-039: E1 re-frozen from a governed observation; no pattern inference."""
    from dynamic_ai_products.collection.documentation_receipt import (
        FROZEN_ENTRY_IDENTITIES as HISTORICAL,
    )

    assert FROZEN_ENTRY_IDENTITIES[0]["final_url"] == (
        "https://docs.cloud.google.com/vertex-ai/generative-ai/docs/thinking"
    )
    assert FROZEN_ENTRY_IDENTITIES[0]["requested_url"] == HISTORICAL[0]["requested_url"]
    assert FROZEN_ENTRY_IDENTITIES[1:] == HISTORICAL[1:], "E2/E3 copied unchanged"
    # Still exactly one hop for every pair.
    for entry in FROZEN_ENTRY_IDENTITIES:
        assert entry["requested_url"] != entry["final_url"]


def test_v3_preserves_every_v2_rule_exactly():
    """The equivalence proof: only routes and contract identity differ.

    Generated-by-substitution modules can drift the moment someone edits one of
    them. This normalises both schemas by replacing route strings, the contract
    id, the schema id, the version const and the prose comment, then requires
    byte equality -- so any change to a bound, pattern, phase, reason,
    disposition or conditional in one and not the other fails here.
    """
    import re
    ...
    def normalise(schema, ids, contract, schema_id, version):
        text = json.dumps(schema, sort_keys=True)
        for index, entry in enumerate(ids):
            text = text.replace(entry["requested_url"], f"<REQ{index}>")
            text = text.replace(entry["final_url"], f"<FIN{index}>")
        text = text.replace(contract, "<CONTRACT>").replace(schema_id, "<SCHEMA_ID>")
        text = text.replace(f'"const": "{version}"', '"const": "<VERSION>"')
        return re.sub(r'"comment": ".*?(?<!\\)"', '"comment": "<COMMENT>"', text)

    left = normalise(expected_prev(), HISTORICAL,
                     "documentation_collection_receipt@0.2.0",
                     "documentation_collection_receipt.v2.schema.json", "0.2.0")
    right = normalise(v3.expected_receipt_schema_v3(), FROZEN_ENTRY_IDENTITIES,
                      v3.RECEIPT_CONTRACT_V3, v3.RECEIPT_SCHEMA_ID_V3, "0.3.0")
    assert left == right


def test_v3_reuses_the_v2_vocabularies_unchanged():
    from dynamic_ai_products.collection import documentation_receipt_v2 as prev

    assert v3.FAILURE_PHASES == prev.FAILURE_PHASES
    assert v3.REASON_PHASES == prev.REASON_PHASES
    assert v3.LOCATION_DISPOSITIONS == prev.LOCATION_DISPOSITIONS
    assert v3.LOCATION_MAX_LENGTH == prev.LOCATION_MAX_LENGTH
    assert v3.ENTRY_REQUIRED_V3 == prev.ENTRY_REQUIRED_V2
    assert v3.PRINTABLE_ASCII_REQUIRED_PATTERN == prev.PRINTABLE_ASCII_REQUIRED_PATTERN
    assert v3.NON_PRINTABLE_ASCII_PATTERN == prev.NON_PRINTABLE_ASCII_PATTERN
...
def test_a_weakened_schema_is_refused(mutate, expected):
    schema = copy.deepcopy(v3.expected_receipt_schema_v3())
    mutate(schema)
    with pytest.raises(CollectionError) as excinfo:
        v3.validate_receipt_schema_v3_bytes(_serialize(schema))
...
def test_a_duplicate_member_name_is_refused_at_any_depth():
    text = V3_BYTES.decode("utf-8")
    doubled = text.replace('"type": "object",', '"type": "object",\n  "type": "object",', 1)
    with pytest.raises(CollectionError) as excinfo:
        v3.validate_receipt_schema_v3_bytes(doubled.encode("utf-8"))
...
def test_unusable_schema_bytes_are_refused(payload):
    with pytest.raises(CollectionError):
        v3.validate_receipt_schema_v3_bytes(payload)


def test_formatting_is_outside_the_comparison():
    compact = json.dumps(v3.expected_receipt_schema_v3(), separators=(",", ":")).encode()
    assert v3.validate_receipt_schema_v3_bytes(compact) == sha256(compact).hexdigest()
...
def test_the_entry_carries_exactly_twenty_locked_fields():
    assert len(v3.ENTRY_REQUIRED_V3) == 20
    assert "http_status" not in v3.ENTRY_REQUIRED_V3, "removed by 0.2.0"
    assert "redirect_chain" not in v3.ENTRY_REQUIRED_V3, "renamed to request_chain"
    assert "request_chain" in v3.ENTRY_REQUIRED_V3
    for field in (
        "failure_phase",
        ...
        assert field in v3.ENTRY_REQUIRED_V3, field
    entry_schema = v3.expected_receipt_schema_v3()["properties"]["entries"]["prefixItems"][0]
    assert entry_schema["additionalProperties"] is False
    assert set(entry_schema["properties"]) == set(v3.ENTRY_REQUIRED_V3)
    assert sorted(entry_schema["required"]) == sorted(v3.ENTRY_REQUIRED_V3)
...
def test_request_chain_is_pinned_to_three_constants_per_entry():
    ...
    for index, frozen in enumerate(FROZEN_ENTRY_IDENTITIES):
        prefix = v3.expected_receipt_schema_v3()["properties"]["entries"]["prefixItems"][index]
...
def test_every_entry_recordable_reason_maps_to_declared_phases():
    assert set(v3.REASON_PHASES) == set(ENTRY_RECORDABLE_REASONS)
    assert len(v3.REASON_PHASES) == 20
    for reason, phases in v3.REASON_PHASES.items():
        assert phases, reason
        for phase in phases:
            assert phase in v3.FAILURE_PHASES, (reason, phase)
...
def test_each_reason_builds_and_validates_in_each_of_its_phases(reason):
    for phase in v3.REASON_PHASES[reason]:
...
def test_a_phase_cannot_claim_an_observation_it_never_had(phase, field):
    reason = v3.REASON_PHASES and next(
        r for r, ps in v3.REASON_PHASES.items() if phase in ps
    )
...
def test_the_location_transcription_table(value, received, expected_value, expected_disposition):
    got, disposition = v3.classify_observed_location(value, response_received=received)
...
def test_an_untranscribable_location_is_never_truncated():
    ...
    got, disposition = v3.classify_observed_location(long_value, response_received=True)
    assert got is None and disposition == "rejected_oversize"
    assert got != long_value[: v3.LOCATION_MAX_LENGTH]


def test_a_location_at_exactly_the_limit_is_recorded():
    value = "h" * v3.LOCATION_MAX_LENGTH
    got, disposition = v3.classify_observed_location(value, response_received=True)
...
def test_the_measured_httpx_join_of_duplicate_field_lines_is_transcribable():
    ...
    got, disposition = v3.classify_observed_location(joined, response_received=True)
...
def test_an_ascii_space_only_location_is_absent(blank):
    ...
    assert v3.classify_observed_location(blank, response_received=True) == (None, "absent")
...
def test_no_whitespace_value_is_ever_recorded_or_truncated(value):
    ...
    assert not v3.transcribable_location(value)
...
def test_the_locked_disposition_order_is_length_then_charset_then_space():
    ...
    over = "x" * (v3.LOCATION_MAX_LENGTH + 1)
    ...
    assert v3.classify_observed_location(over + "\n", response_received=True) == (
        None, "rejected_oversize",
    )
    # Charset wins over space-only.
    assert v3.classify_observed_location(" \t ", response_received=True) == (
        None, "rejected_uncharacterizable",
    )
    # Space-only is reached only once charset has passed.
    assert v3.classify_observed_location("  ", response_received=True) == (None, "absent")
...
def test_classify_and_transcribable_agree_exactly():
    ...
    for sample in samples:
        value, disposition = v3.classify_observed_location(sample, response_received=True)
        assert (disposition == "recorded") == v3.transcribable_location(sample), sample
...
def test_the_committed_schema_carries_the_binding_branches():
    prefix = json.loads(V3_BYTES.decode("utf-8"))["properties"]["entries"]["prefixItems"][0]
    bound = [
    ...
    for branch in bound:
        ...
        assert recorded["pattern"] == v3.PRINTABLE_ASCII_REQUIRED_PATTERN
        assert recorded["not"] == {"pattern": v3.NON_PRINTABLE_ASCII_PATTERN}
        assert recorded["minLength"] == 1
        assert recorded["maxLength"] == v3.LOCATION_MAX_LENGTH
...
def test_the_committed_schema_uses_no_anchored_location_pattern():
    ...
    for pattern in (v3.PRINTABLE_ASCII_REQUIRED_PATTERN, v3.NON_PRINTABLE_ASCII_PATTERN):
    ...
    assert v3.NON_PRINTABLE_ASCII_PATTERN.startswith("[^")
    assert "^" not in v3.PRINTABLE_ASCII_REQUIRED_PATTERN
...
def test_the_two_predicates_are_individually_search_safe():
    ...
    assert re.search(v3.PRINTABLE_ASCII_REQUIRED_PATTERN, "x\n")
    assert re.search(v3.NON_PRINTABLE_ASCII_PATTERN, "x\n")
    assert re.search(v3.PRINTABLE_ASCII_REQUIRED_PATTERN, "x")
    assert not re.search(v3.NON_PRINTABLE_ASCII_PATTERN, "x")
    # A space-only value fails the positive predicate without needing an anchor.
    assert not re.search(v3.PRINTABLE_ASCII_REQUIRED_PATTERN, "   ")
...
def test_the_committed_schema_and_the_builder_never_diverge(value, label):
    ...
    assert schema_says is v3.transcribable_location(value), label
    assert schema_says is False, label


def test_the_dispositions_are_the_five_declared_values():
    assert v3.LOCATION_DISPOSITIONS == (
        "no_response", "absent", "recorded", "rejected_oversize",
        "rejected_uncharacterizable",
    )
    assert "no_response" not in v3.RESPONSE_DISPOSITIONS
    assert len(v3.RESPONSE_DISPOSITIONS) == 4
...
def test_the_builder_refuses_a_malformed_header(override):
    ...
    with pytest.raises(CollectionError):
        v3.build_documentation_receipt_v3(**kwargs)


def test_every_builder_output_validates_against_the_committed_schema():
    receipt = _build([_succeeded(0), _succeeded(1), _succeeded(2)], "completed")
    committed = Draft202012Validator(json.loads(V3_BYTES.decode("utf-8")))
    assert not list(committed.iter_errors(receipt))
    assert v3.receipt_bytes_v3(receipt).endswith(b"\n")


# --- module hygiene -----------------------------------------------------------


def test_the_v3_module_declares_no_url_literal():
...
def test_the_v3_module_reads_no_clock_and_runs_no_process():
    source = (SRC / "documentation_receipt_v3.py").read_text(encoding="utf-8")
