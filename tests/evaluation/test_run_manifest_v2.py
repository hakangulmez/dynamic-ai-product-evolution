...
def test_no_static_schema_added_and_schema_manifest_unchanged():
    ...
    # rebaselined (latest: ADR-130, 0.68.0 -> 0.69.0, 164 -> 172 entries,
    # registering the 0.3.0 axes and record contracts and six V2.4
    # authorization/manifest contracts; before it ADR-129, 0.67.0 -> 0.68.0,
    # 158 -> 164 entries,
    # registering the six V2.3 authorization and manifest contracts -- a
    # prompt-discipline successor that keeps the 0.2.0 axes and record
    # contracts, the taxonomy version and the 12/1200 ceilings byte-unchanged
    # and moves only prompt_template_path, which is a const; before it
    # ADR-128, 0.66.0 -> 0.67.0, 150 -> 158 entries,
    # registering the 0.2.0 axes and record contracts and the six V2.2
    # authorization and manifest contracts -- two output ceilings widened
    # (evidence 6 -> 12, quote 300 -> 1200) after a live calibration was
    # refused on size alone; the economic axes and the tier rules are
    # byte-identical and every 0.1.0 contract stays byte-unchanged; before it
    # ADR-127, 0.65.0 -> 0.66.0, 146 -> 150 entries,
    ...
    # from a separately versioned rule config; every earlier contract and the
    # V5 screen prompt stay byte-unchanged; before it
    # ADR-112, 0.49.0 -> 0.50.0, 105 -> 108 entries,
    # registering the three diagnostic-canary contracts (record,
    # manifest, authorization) -- every authoritative screen contract
    # and prompt stays byte-unchanged; before it
    # ADR-111, 0.48.0 -> 0.49.0, 104 -> 105 entries,
    # registering universe_screen_manifest_v4, the strict live-manifest
    # successor pinning the v3 screen prompt -- the v1/v2 prompts and the
    # v0.1-v0.3 screen manifests stay byte-unchanged; before it
    # ADR-110, 0.47.0 -> 0.48.0, 103 -> 104 entries,
    # registering universe_screen_manifest_v3, the strict live-manifest
    # successor pinning the v2 screen prompt -- the v1 prompt and the
    # v0.1/v0.2 screen manifests stay byte-unchanged; before it
    # ADR-109, 0.46.0 -> 0.47.0, 99 -> 103 entries,
    ...
    # ADR-107, 0.44.0 -> 0.45.0, 96 -> 97 entries,
    # registering baseline_packet_manifest_v5, the two-determination lineage
    # packet-run manifest -- every predecessor packet schema stays
    # byte-unchanged; before it
    # ADR-105, 0.43.0 -> 0.44.0, 94 -> 96 entries,
    # registering the asset-backed-issuer determination record and its
    # lineage run manifest -- the shell-determination schemas and every
    # packet contract stay byte-unchanged; before it
    # ADR-104, 0.42.0 -> 0.43.0, 93 -> 94 entries,
    # registering baseline_packet_manifest_v4, the locator-declared lineage
    # packet-run manifest -- the v0.2 packet record contract and every
    # predecessor schema stay byte-unchanged; before it
    # ADR-103, 0.41.0 -> 0.42.0, 92 -> 93 entries,
    # registering baseline_packet_manifest_v3, the lineage-cohort packet-run
    # manifest -- the v0.2 packet record contract and every predecessor
    # schema stay byte-unchanged; before it
    # ADR-102, 0.40.0 -> 0.41.0, 91 -> 92 entries,
    # registering shell_company_determination_manifest_v3, the lineage-cohort
    # run manifest -- the v0.2 determination record contract and every
    # predecessor schema stay byte-unchanged; before it
    # ADR-101, 0.39.0 -> 0.40.0, 90 -> 91 entries,
    # registering acquisition_queue_aggregate_manifest_v2, the lineage
    # aggregate successor scoped to enumerated execution run ids -- the v0.1
    # aggregate schema stays byte-unchanged; before it
    # ADR-099, 0.38.0 -> 0.39.0, 88 -> 90 entries,
    # registering pct_dev30_v0_model_output_v2 and
    # pct_dev30_v0_persisted_candidates_v2, the Dev30 v0.2
    # quote-derived-locator successor -- v0.1's two schemas stay
    # byte-unchanged; before it
    # rebaselined (latest: ADR-098, 0.37.0 -> 0.38.0, 86 -> 88 entries,
    # registering pct_dev30_v0_model_output and
    # pct_dev30_v0_persisted_candidates for the Dev30 combined-candidate
    # contract -- not added to EVALUATION_SCHEMA_CONTRACTS or
    # RELEASED_EVALUATION_CONTRACTS, since Dev30 is not a production
    # evaluation-harness contract; before it
    # rebaselined (latest: ADR-097, 0.36.0 -> 0.37.0, 81 -> 86 entries,
    # registering the plain-text successors: bundle v0.2, packet v0.2,
    # packet manifest v0.2, queue definition v0.2 and acquisition manifest
    # v0.6; before it
    # rebaselined (latest: ADR-095, 0.35.0 -> 0.36.0, 75 -> 81 entries,
    # registering the four acquisition-queue schemas and the budgeted
    # fixture-v0.4 and sec_live-v0.5 acquisition-manifest successors;
    # before it
    # rebaselined (latest: the ADR-094 transform correction, 0.34.0 ->
    # 0.35.0, 73 -> 75 entries, registering the v2 shell-company
    # determination record and run-manifest successors at 0.2.0 for
    # ixt:booleantrue and ixt:fixed-true, with the v0.1 pair retained
    # unchanged at 0.1.0;
    # before it ADR-094, 0.33.0 -> 0.34.0, registering the
    # shell-company determination record and its run manifest; before it
    # ADR-093, 0.32.0 -> 0.33.0, registering the v3 observational
    # sec_live successor for primary-document acquisition;
    # before it ADR-092, 0.31.0 -> 0.32.0, registering the
    # primary-document acquisition manifest and its v2 sec_live successor;
    # before it ADR-091, 0.30.0 -> 0.31.0, registering the baseline
    # primary-document bundle, the baseline packet and its run manifest; before it ADR-090, 0.29.0 -> 0.30.0,
    # registering filing_index_probe_manifest and its v2 successor; before it ADR-089, 0.28.0 -> 0.29.0,
    # registering baseline_document_acquisition_manifest and its v2 successor
    # for W2-B baseline filing-document acquisition; before it ADR-088,
    # 0.27.0 -> 0.28.0, registering
    ...
    assert got == "2ffac734b8d2fc2a1585675362f6e188145a5ad80b9f3df6d8ead90395e1e33c"
