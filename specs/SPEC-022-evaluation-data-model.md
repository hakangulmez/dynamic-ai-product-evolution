...
## Observation-target binding versions (ADR-026, ADR-029)

`observation_target_binding@0.1.0` (ADR-026) remains the only artifact permitted to map raw extraction observation IDs onto canonical target references; its fields, generated schema, governed contract hash, and capability-stage bytes are unchanged, and its paired parent-snapshot pins stay present exactly when a parent-referenced entry exists. `observation_target_binding@0.2.0` (ADR-029) is the explicit task-stage successor: an identical field set except that `parent_observation_snapshot_version` and `parent_observation_snapshot_sha256` are required non-null, durably binding the exact committed snapshot identity task-stage Rule 7 consumed. The successor contract accepts only `task_extraction`; version selection is a strict declared-contract-version peek with no fallback or coercion, the loaded wrapper carries the closed v0.1|v0.2 union, and every revalidation, persistence, and accessor path discriminates fail-closed on the concrete contract.
...
- 2026-07-27 — Revised per ADR-029: explicit `observation_target_binding@0.2.0` task-stage successor with required parent-snapshot pins; `observation_target_binding@0.1.0` protected unchanged.
