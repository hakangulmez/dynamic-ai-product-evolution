...
Rule 10's `active_status_values` and `roadmap_status_values` are an **evaluator classification**: at the `capability_extraction` and `task_extraction` stages they determine whether a record is supported only by future-roadmap evidence. `product_candidate_availability_vocabulary@0.1.0` (ADR-052) is a **candidate-admission vocabulary**: at the `product_extraction` stage it determines which statuses may enter a candidate record at all. The two are separate contracts with separate consumers. Neither is derived from the other, they are not required to carry the same tokens, and neither substitutes for the other. `product_extraction` is not part of the governed four-stage evaluator matrix and is not added to it.
...
- 2026-08-04 — Revised per ADR-052: one paragraph recording that Rule 10's availability-status vocabularies are an evaluator classification and are separate from the `product_extraction` candidate-admission vocabulary. No rule text, stage matrix, applicability table, blocking-reason vocabulary, per-rule hash or aggregate hash changes.
